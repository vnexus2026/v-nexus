          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (val === "" || isPrefix) {
                ...
                alert(
                  "請將「V-Nexus審核中」暫時加入你的X或YT簡介，並告知管理員你放在X還是YT即可，請不要輸入其他訊息。",
                );
              }
            }}
            className={inputCls}
...
const HomePage = ({
  ...
  }, [safeBulletins, realVtubers, currentTime]);
  ...
  return (
    <section className="pt-16 pb-20 px-4 text-center max-w-5xl mx-auto animate-fade-in-up">
      ...
      <p className="text-xs text-gray-500 font-medium tracking-widest mb-16 -mt-2">
...
const AdminVtuberList = ({
  ...
  useEffect(() => {
    if (paginate && page > totalPages && totalPages > 0) setPage(totalPages);
  }, [filteredList.length, totalPages, page, paginate]);
  ...
  return (
    <div className="mb-8">
      ...
      {filteredList.length === 0 && (
        <p className="text-gray-500 text-sm">目前無資料</p>
      )}
      <div className="space-y-3">
        {displayList.map((v) => (
          <div
            key={v.id}
            className={`flex items-center justify-between p-4 rounded-xl border ${isBlacklist ? "bg-red-950/20 border-red-900/50" : title.includes("待") ? "bg-gray-900 border-yellow-500/30" : "bg-gray-900 border-gray-700"} gap-4`}
          >
            <div className="flex items-center gap-4">
              <img
                src={sanitizeUrl(v.avatar)}
              ...
              <div>
                ...
                {privateDocs[v.id]?.verificationNote && (
                  <p className="text-xs text-yellow-400 mt-1.5 bg-yellow-500/10 p-1.5 rounded border border-yellow-500/20 inline-block">
                    <i className="fa-solid fa-key mr-1"></i> 驗證備註:{" "}
                    {privateDocs[v.id].verificationNote}
                  </p>
                )}
                ...
                <div className="flex gap-3 mt-1.5">
                  {(v.youtubeUrl || v.channelUrl) && (
                    ...
                    >
                      <i className="fa-brands fa-youtube"></i> 檢查
                    </a>
                  )}
                  {v.twitchUrl && (
                    <a
                    ...
                    >
                      <i className="fa-brands fa-twitch"></i> 檢查
                    </a>
                  )}
                  {v.xUrl && (
                    <a
                    ...
                    >
                      <i className="fa-brands fa-x-twitter"></i> 檢查
                    </a>
                  )}
                </div>
              </div>
            ...
            <div className="flex gap-2 flex-shrink-0">
              {onVerify && (
                <button
                  ...
                  className="text-green-400 bg-green-500/10 px-4 py-2 rounded-lg text-sm font-bold"
                >
                  審核
                </button>
              )}
              {onReject && (
                <button
                  ...
                  className="text-orange-400 bg-orange-500/10 px-4 py-2 rounded-lg text-sm font-bold"
                >
                  拒絕
                </button>
              )}
              <button
                onClick={() => onEdit(v)}
                className="text-blue-400 bg-blue-500/10 px-3 py-2 rounded-lg text-sm font-bold"
              >
                編輯
              </button>
              <button
                onClick={() => onDelete(v.id)}
                className={`text-red-400 px-3 py-2 rounded-lg text-sm font-bold ${isBlacklist ? "bg-red-900/50 hover:bg-red-600 hover:text-white" : "bg-red-500/10"}`}
              >
                刪除
              </button>
            </div>
...
const AdminPage = ({
  ...
  const handleTestMail = async () => {
    try {
      ...
      showToast(
        "✅ 測試信件任務已寫入！請去 Firebase 後台查看 mail 路徑是否出現。",
      );
    } catch (err) {
...
function App() {
  ...
  useEffect(() => {
    const fetchLargeData = async () => {
      ...
      if (needsActivityData) {
        ...
        const isBCacheValid =
          bCache && bTs && now - parseInt(bTs) < ACTIVITY_CACHE_LIMIT;
        const isCCacheValid =
          cCache && cTs && now - parseInt(cTs) < ACTIVITY_CACHE_LIMIT;
        const forceRefresh = currentView === "admin";
  ...
  const handleSaveProfile = async (e, customForm = profileForm) => {
    ...
    if (!isAdmin) {
      ...
      if (note !== "X" && note !== "YT") {
        alert(
          "請將「V-Nexus審核中」暫時加入你的X或YT簡介，並告知管理員你放在X還是YT即可。",
        );
        return;
      }
    }
    ...
    try {
      ...
      if (isApplying && bulletinAuthorId !== user.uid) {
        ...
        addDoc(collection(db, getPath("notifications")), {
          ...
          fromUserAvatar: myProfile?.avatar || user.photoURL,
          message:
            "有人有意願！快去招募佈告欄看看！快回V-NEXUS看看！https://www.vnexus2026.com/",
          createdAt: Date.now(),
          read: false,
        }).catch(() => {});
      }
  ...
  const handleDeleteVtuber = async (id) => {
    ...
    try {
      ...
      setRealVtubers((prev) => prev.filter((v) => v.id !== id));
      showToast("已刪除名片");
    } catch (err) {
      showToast("刪除失敗");
    }
  ...
  const handleDeleteBulletin = async (id) => {
    ...
    try {
      ...
      setRealBulletins((prev) => prev.filter((b) => b.id !== id));
      showToast("已刪除招募文");
    } catch (err) {
      showToast("刪除失敗");
    }
  ...
  const handleDeleteCollab = async (id) => {
    ...
    try {
      ...
      setRealCollabs((prev) => prev.filter((c) => c.id !== id));
      showToast("已刪除聯動行程");
    } catch (err) {
      showToast("刪除失敗");
    }
      ...
      <nav className="sticky top-0 z-40 backdrop-blur-md bg-[#0f111a]/95 border-b border-gray-800 shadow-xl">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="hidden lg:flex items-center justify-center gap-4 pb-4 overflow-x-auto whitespace-nowrap scrollbar-hide">
            ...
            <button
              ...
              className={`transition-colors px-3 py-1.5 font-bold text-sm whitespace-nowrap ${currentView === "blacklist" ? "text-red-500 border-b-2 border-red-600" : "text-red-400/70 hover:text-red-400"}`}
            >
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden absolute top-16 left-0 w-full bg-[#0f111a]/95 backdrop-blur-md border-b border-gray-800 shadow-xl flex flex-col p-4 gap-4 z-50 animate-fade-in-up">
            ...
            <button
              onClick={() => navigate("blacklist")}
              className={`text-left px-4 py-3 rounded-xl font-bold flex items-center gap-3 ${currentView === "blacklist" ? "bg-red-900/50 text-red-400" : "text-red-400/70 hover:bg-red-900/30"}`}
            >
              <i className="fa-solid fa-ban w-5"></i> 黑單避雷區
            </button>
            {user && (
              <button
                onClick={() => navigate("inbox")}
      ...
      <main className="flex-1 pb-10">
        {currentView === "home" && (
          <HomePage
            ...
            user={user}
            isVerifiedUser={isVerifiedUser}
            onApplyBulletin={handleApplyBulletin}
          />
        )}
