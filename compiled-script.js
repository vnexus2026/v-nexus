...
const VNEXUS_IMAGE_FALLBACK_SRC = "data:image/svg+xml;charset=UTF-8,%3Csvg xmlns='http://www.w3.org/2000/svg' width='16' height='16' viewBox='0 0 16 16'%3E%3Crect width='16' height='16' fill='transparent'/%3E%3C/svg%3E";
function applyVnexusImageFallback(target, failedSrc = "", options = {}) {
    if (!target || target.tagName !== "IMG")
        return;
    const img = target;
    const src = failedSrc || img.currentSrc || img.src || img.getAttribute("src") || "";
    if (src && options.remember !== false)
        rememberBrokenImageUrl(src);
    img.classList.add("vnexus-image-error", "vnexus-local-img-failed");
    img.classList.remove("vnexus-image-loaded");
    img.removeAttribute("srcset");
    img.style.display = "block";
    img.style.opacity = "1";
    img.style.visibility = "visible";
    img.style.backgroundColor = "#1D2130";
    img.style.backgroundImage = "linear-gradient(135deg, #11131C 0%, #1D2130 100%)";
    img.style.color = "transparent";
    img.style.objectFit = img.style.objectFit || "cover";
    if (img.parentElement) {
        img.parentElement.classList.add("vnexus-image-fallback-surface");
    }
    if (img.getAttribute("src") !== VNEXUS_IMAGE_FALLBACK_SRC) {
        img.setAttribute("src", VNEXUS_IMAGE_FALLBACK_SRC);
    }
}
...
const initVnexusImageLoadingUX = (() => {
    ...
    return () => {
        ...
        .vnexus-image-fallback-surface {
          background-color: #1D2130 !important;
          background-image: linear-gradient(135deg, #11131C 0%, #1D2130 100%) !important;
        }
        img.vnexus-image-error,
        img.vnexus-local-img-failed {
          opacity: 1 !important;
      ...
      `;
        ...
        document.addEventListener("load", (event) => {
            ...
            if (target && target.tagName === "IMG") {
                ...
                if (!target.classList.contains("vnexus-local-img-failed")) {
        ...
        document.addEventListener("error", (event) => {
            ...
            if (target && target.tagName === "IMG") {
                ...
                if (target.closest?.(".vnexus-creator-market-card")) {
                    applyVnexusImageFallback(target, failedSrc, { remember: false });
                    return;
                }
                applyVnexusImageFallback(target, failedSrc);
            }
        }, true);
        const hideKnownBrokenImages = (root = document) => {
            root.querySelectorAll?.("img").forEach((img) => {
                ...
                if (src && isKnownBrokenImageUrl(src)) {
                    applyVnexusImageFallback(img, src, { remember: false });
                }
            });
        };
                ...
                mutations.forEach((mutation) => {
                    mutation.addedNodes.forEach((node) => {
                        ...
                        if (node.tagName === "IMG") {
                            ...
                            if (src && isKnownBrokenImageUrl(src)) {
                                applyVnexusImageFallback(node, src, { remember: false });
                            }
                        }
                        else {
...
const LazyImage = ({ src, containerCls = "", imgCls = "", alt = "", onClick, }) => {
    ...
    return (React.createElement("div", { onClick: onClick, className: `relative overflow-hidden bg-[#1D2130] ${containerCls}`, "aria-label": alt || "圖片" },
        ...
        safeSrc && !hasError && (React.createElement("img", { src: safeSrc, alt: alt, className: `relative z-10 w-full h-full object-cover transition-opacity duration-300 ease-out ${loaded ? "opacity-100" : "opacity-0"} ${imgCls}`, onLoad: (e) => {
                ...
                applyVnexusImageFallback(e.currentTarget, safeSrc || e.currentTarget.currentSrc || e.currentTarget.src);
                setHasError(true);
                setLoaded(true);
...
const DEFAULT_SCHEDULE_SLOT = { day: "星期一", start: "20:00", end: "22:00" };
const normalizeScheduleSlot = (slot = {}) => ({
    day: String(slot.day || DEFAULT_SCHEDULE_SLOT.day).trim() || DEFAULT_SCHEDULE_SLOT.day,
    start: String(slot.start || DEFAULT_SCHEDULE_SLOT.start).trim() || DEFAULT_SCHEDULE_SLOT.start,
    end: String(slot.end || DEFAULT_SCHEDULE_SLOT.end).trim() || DEFAULT_SCHEDULE_SLOT.end,
});
const normalizeScheduleFields = (form = {}) => {
    const isAnytime = !!form.isScheduleAnytime;
    const isCustom = !isAnytime && !!form.isScheduleCustom;
    const isExcept = !isAnytime && !isCustom && !!form.isScheduleExcept;
    const rawSlots = Array.isArray(form.scheduleSlots) ? form.scheduleSlots : [];
    const normalizedSlots = rawSlots
        .map((slot) => normalizeScheduleSlot(slot))
        .filter((slot) => slot.day && slot.start && slot.end);
    return {
        isScheduleAnytime: isAnytime,
        isScheduleExcept: isExcept,
        isScheduleCustom: isCustom,
        customScheduleText: isCustom ? String(form.customScheduleText || "").trim() : "",
        scheduleSlots: isAnytime || isCustom ? [] : normalizedSlots,
    };
};
...
const BulletinCard = React.memo(({ b, user, isVerifiedUser, onNavigateProfile, onApply, onInvite, onDeleteBulletin, onEditBulletin, openModalId, onClearOpenModalId, currentView, onViewMore, }) => {
        ...
        : currentView === "home"
            ? React.createElement("div", { className: "vnexus-home-bulletin-card-cta" },
                React.createElement("button", { onClick: (event) => {
                    ...
                    }, className: `vnexus-home-bulletin-primary-cta ${hasApplied ? "is-applied" : ""}` }, hasApplied ? "已送出意願" : "我有意願"),
                React.createElement("button", { onClick: (event) => {
                        event.preventDefault();
                        event.stopPropagation();
                        if (onViewMore)
                            onViewMore(b);
                    }, className: "vnexus-home-bulletin-secondary-cta" }, "看詳細"))
            : React.createElement("button", { onClick: (event) => {
                    event.preventDefault();
                    event.stopPropagation();
                    onApply(b.id, !hasApplied, b.userId);
    ...
    const applicantsPortal = showApplicants && currentView !== "profile" && ReactDOM.createPortal(React.createElement("div", { className: "fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/75", onClick: (event) => { event.stopPropagation(); setShowApplicants(false); } },
        React.createElement("div", { className: "bg-[#0F111A] border border-[#2A2F3D] rounded-2xl w-full max-w-md flex flex-col max-h-[80vh] shadow-sm cursor-default", onClick: (event) => event.stopPropagation() },
            ...
            React.createElement("div", { className: "p-4 overflow-y-auto space-y-3" }, b.applicantsData?.length > 0 ? b.applicantsData.map((a) => (React.createElement("div", { key: a.id, className: "flex items-center justify-between bg-[#181B25]/50 p-3 rounded-xl border border-[#2A2F3D] hover:border-white/10 transition-colors cursor-pointer group", onClick: (event) => { event.stopPropagation(); setShowApplicants(false); onNavigateProfile(a, true); } },
                React.createElement("div", { className: "flex items-center gap-4 min-w-0" },
                    React.createElement("img", { src: sanitizeUrl(a.avatar), className: "w-12 h-12 rounded-full object-cover border border-[#2A2F3D] bg-[#1D2130]", onError: (event) => { applyVnexusImageFallback(event.currentTarget); } }),
                    React.createElement("div", { className: "min-w-0" },
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: "h-full bg-[#181B25] border border-[#2A2F3D] rounded-2xl overflow-hidden flex flex-col hover:bg-[#1D2130] hover:border-white/10 transition-colors shadow-sm" },
                    ...
                    React.createElement("div", { className: "flex items-center justify-between gap-3" },
                        React.createElement("button", { onClick: (event) => { event.preventDefault(); event.stopPropagation(); setShowApplicants(true); }, className: "min-w-0 flex items-center gap-2 text-left rounded-xl hover:bg-[#11131C] transition-colors p-2 -m-2" },
                            React.createElement("div", { className: "flex -space-x-2 flex-shrink-0" },
                                b.applicantsData?.slice(0, 3).map((a) => (React.createElement("img", { key: a.id, src: sanitizeUrl(a.avatar), className: "w-7 h-7 rounded-full ring-2 ring-[#181B25] object-cover bg-[#1D2130]", onError: (event) => { applyVnexusImageFallback(event.currentTarget); } }))),
...
const ScheduleEditor = ({ form, updateForm }) => {
    const mode = form.isScheduleAnytime
        ...
        : form.isScheduleExcept
            ...
            : form.isScheduleCustom
                ? "custom"
                : "specific";
    const getSafeSlots = () => (Array.isArray(form.scheduleSlots) ? form.scheduleSlots : []).map((slot) => normalizeScheduleSlot(slot));
    const handleModeChange = (e) => {
        ...
        const nextBase = {
            ...
            scheduleSlots: (m === "specific" || m === "except") && currentSlots.length === 0
                ? [{ ...DEFAULT_SCHEDULE_SLOT }]
                : currentSlots,
        };
        updateForm(normalizeScheduleFields(nextBase));
    };
    const updateSlot = (idx, field, val) => {
        const newSlots = getSafeSlots();
        if (!newSlots[idx])
            return;
    ...
    const removeSlot = (idx) => {
        updateForm({ scheduleSlots: getSafeSlots().filter((_, i) => i !== idx) });
    };
    const addSlot = () => {
        updateForm({
            isScheduleAnytime: false,
            isScheduleCustom: false,
            scheduleSlots: [...getSafeSlots(), { ...DEFAULT_SCHEDULE_SLOT }],
        });
    };
    return (React.createElement("div", { className: "bg-[#0F111A] border border-[#2A2F3D] rounded-xl p-4 mt-1" },
        ...
        (mode === "specific" || mode === "except") && (React.createElement("div", { className: "space-y-3 border-t border-[#2A2F3D] pt-3" },
            getSafeSlots().map((slot, index) => (React.createElement("div", { key: index, className: "flex flex-wrap items-center gap-2 bg-[#181B25]/50 p-2 rounded-lg border border-[#2A2F3D]" },
                ...
                React.createElement("select", { value: slot.end, onChange: (e) => updateSlot(index, "end", e.target.value), className: "bg-[#181B25] border border-[#2A2F3D] rounded p-1.5 text-white text-xs outline-none box-border min-w-0 cursor-pointer" },
                    ...
                    TIME_OPTIONS.map(t => React.createElement("option", { key: t, value: t }, t))),
                React.createElement("button", { type: "button", onClick: () => removeSlot(index), className: "text-[#EF4444] hover:text-red-300 ml-auto px-2 transition-colors" },
                    React.createElement("i", { className: "fa-solid fa-trash" }))))),
            React.createElement("button", { type: "button", onClick: addSlot, className: "text-xs text-[#A78BFA] hover:text-white flex items-center gap-1 mt-2 bg-[#8B5CF6]/10 hover:bg-[#8B5CF6] px-3 py-1.5 rounded-lg border border-[#8B5CF6]/20 transition-colors" },
                ...
                React.createElement("div", { className: "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5" }, pagedRequests.map((r) => {
                    ...
                    return (React.createElement("article", { key: r.id, className: "bg-[#181B25] border border-[#2A2F3D] rounded-2xl p-5 shadow-sm hover:bg-[#1D2130] transition-colors h-full flex flex-col" },
                        React.createElement("div", { className: "flex items-start gap-3 mb-4" },
                            React.createElement("button", { onClick: () => author && openProfile(author), className: "w-12 h-12 rounded-2xl bg-[#11131C] border border-[#2A2F3D] overflow-hidden flex-shrink-0", title: "\u67E5\u770B\u767C\u6848\u8005\u540D\u7247" }, author?.avatar ? React.createElement("img", { src: sanitizeUrl(author.avatar), alt: author?.name || "發案者", className: "vnexus-critical-avatar-img w-full h-full object-cover", onError: (e) => { applyVnexusImageFallback(e.currentTarget); } }) : React.createElement("div", { className: "w-full h-full flex items-center justify-center text-[#64748B]" },
...
const HomePage = ({ navigate, onOpenRules, onOpenUpdates, hasUnreadUpdates, siteStats = { pageViews: null }, realCollabs = [], displayCollabs = [], currentTime = Date.now(), isLoadingCollabs, goToBulletin, registeredCount, realVtubers = [], setSelectedVTuber, realBulletins = [], onShowParticipants, user, isVerifiedUser, onApply, onNavigateProfile, onOpenStoryComposer, onSubmitFeedback, showToast, }) => {
    ...
    const TaskEntryCard = ({ icon, title, desc, cta, onClick, tone = "purple" }) => (React.createElement("button", { type: "button", onClick: onClick, className: `vnexus-task-entry-card vnexus-task-entry-card--${tone}` },
        React.createElement("div", { className: "vnexus-task-entry-icon" },
            React.createElement("i", { className: `fa-solid ${icon}` })),
        React.createElement("div", { className: "vnexus-task-entry-body" },
            React.createElement("p", { className: "vnexus-task-entry-title" }, title),
            React.createElement("p", { className: "vnexus-task-entry-desc" }, desc),
            React.createElement("span", { className: "vnexus-task-entry-cta" },
                cta,
    ...
    return (React.createElement("section", { className: "pt-10 md:pt-14 pb-20 px-4 max-w-6xl mx-auto animate-fade-in-up" },
                ...
                React.createElement("div", { className: "mt-7 space-y-3" },
                    ...
                    React.createElement("button", { type: "button", onClick: handleOpenFeedbackReport, className: "w-full min-h-12 bg-[#22C55E]/10 hover:bg-[#22C55E]/15 text-[#86EFAC] border border-[#22C55E]/30 px-5 py-3 rounded-xl font-bold transition-colors flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-2 text-center" },
                        React.createElement("span", null,
                            ...
                            "\u529F\u80FD\u56DE\u994B\u53CA\u554F\u984C\u56DE\u5831"),
                        React.createElement("span", { className: "text-[11px] sm:text-xs font-medium text-[#BBF7D0]" }, "\u6709\u4EFB\u4F55\u60F3\u8981\u7684\u529F\u80FD\u6216\u554F\u984C\uFF0C\u8ACB\u76F4\u63A5\u586B\u5BEB\u4E26\u56DE\u5831\u3002")))),
                                ...
                                React.createElement("button", { type: "button", onClick: openStoryProfile, className: "vnexus-story-avatar-button block w-full", "aria-label": `查看 ${v.name || "VTuber"} 的名片` },
                                    React.createElement("div", { className: `w-12 h-12 mx-auto rounded-full p-[2px] ${isLiveMsg ? "bg-[#EF4444]" : "bg-[#F59E0B]"}` },
                                        React.createElement("div", { className: "w-full h-full rounded-full bg-[#11131C] p-[2px]" },
                                            React.createElement("img", { src: sanitizeUrl(v.avatar), className: "w-full h-full rounded-full object-cover bg-[#1D2130]", onError: (e) => { applyVnexusImageFallback(e.currentTarget); }, alt: v.name || "VTuber" })))),
        ...
        React.createElement("div", { className: "vnexus-home-task-section", "aria-label": "首頁快速任務入口" },
            ...
            React.createElement("div", { className: "vnexus-home-task-grid" },
                ...
                React.createElement(TaskEntryCard, { tone: "purple", icon: "fa-palette", title: "我想找繪師/建模師/剪輯師", desc: "瀏覽繪師、建模師、剪輯師的接案資訊與作品集，先找到適合的創作者。", cta: "進入技術專區", onClick: () => navigate("commissions") }),
                React.createElement(TaskEntryCard, { tone: "sky", icon: "fa-clipboard-list", title: "我要接案 / 找案源 / 發布委託", desc: "發布自己的委託需求，或查看最新案源並送出接案意願。", cta: "進入委託佈告欄", onClick: goToCommissionRequests }))),
        React.createElement("div", { className: "mt-10 pt-10 border-t border-[#2A2F3D] w-full animate-fade-in-up" },
            ...
            randomBulletins.length > 0 ? (React.createElement("div", { className: "flex items-stretch overflow-x-auto pb-6 gap-4 snap-x snap-mandatory md:grid md:grid-cols-3 md:overflow-visible custom-scrollbar" },
                randomBulletins.map((b) => (React.createElement("div", { key: b.id, className: "flex-shrink-0 w-[85vw] md:w-auto snap-center text-left h-full" },
                    React.createElement(BulletinCard, { b: b, user: user, isVerifiedUser: isVerifiedUser, onNavigateProfile: onNavigateProfile, onApply: onApply, currentView: "home", onViewMore: goToBulletin })))),
                    ...
                    return (React.createElement("article", { key: `home-commission-request-${r.id}`, className: "flex-shrink-0 w-[85vw] md:w-auto snap-center bg-[#181B25] border border-[#2A2F3D] rounded-2xl p-5 text-left hover:bg-[#1D2130] transition-colors h-full flex flex-col" },
                        React.createElement("div", { className: "flex items-start justify-between gap-3 mb-4" },
                            React.createElement("div", { className: "flex items-center gap-3 min-w-0" },
                                React.createElement("img", { src: sanitizeUrl(author?.avatar || "https://api.dicebear.com/7.x/avataaars/svg?seed=Anon"), alt: author?.name || "發案者", className: "w-11 h-11 rounded-xl object-cover bg-[#1D2130] border border-[#2A2F3D] flex-shrink-0", onError: (e) => { applyVnexusImageFallback(e.currentTarget); } }),
                                React.createElement("div", { className: "min-w-0" },
                ...
                React.createElement("div", { className: "px-5 py-4 border-b border-[#2A2F3D] bg-[#181B25] flex items-center justify-between gap-3" },
                    React.createElement("div", null,
                        React.createElement("h3", { className: "text-white font-black text-lg flex items-center gap-2" },
                            React.createElement("i", { className: "fa-solid fa-message text-[#22C55E]" }),
                            "\u529F\u80FD\u56DE\u994B\u53CA\u554F\u984C\u56DE\u5831"),
                        React.createElement("p", { className: "text-[#94A3B8] text-xs mt-1" }, "\u4F60\u586B\u5BEB\u7684\u5167\u5BB9\u4E0D\u6703\u516C\u958B\u986F\u793A\uFF0C\u50C5\u4F9B\u7BA1\u7406\u54E1\u5728\u5F8C\u53F0\u67E5\u770B\u3002")),
                ...
                React.createElement("div", { className: "p-5 space-y-4" },
                    ...
                    React.createElement("div", { className: "flex items-center justify-between gap-3" },
                        React.createElement("span", { className: `text-[10px] ${feedbackText.length >= 760 ? "text-[#F59E0B]" : "text-[#94A3B8]"}` },
                            ...
                            " / 800"),
                        React.createElement("div", { className: "flex gap-2" },
                            React.createElement("button", { type: "button", onClick: () => !isSubmittingFeedback && setIsFeedbackModalOpen(false), className: "px-4 py-2 rounded-xl bg-[#181B25] hover:bg-[#1D2130] text-[#CBD5E1] font-bold text-sm transition-colors" }, "\u53D6\u6D88"),
                            React.createElement("button", { type: "submit", disabled: isSubmittingFeedback || !feedbackText.trim(), className: `px-5 py-2 rounded-xl font-black text-sm transition-colors ${feedbackText.trim() && !isSubmittingFeedback ? "bg-[#22C55E] hover:bg-[#16A34A] text-[#0F111A]" : "bg-[#1D2130] text-[#64748B] cursor-not-allowed"}` }, isSubmittingFeedback ? "送出中..." : "送出回報")))))))));
};
...
massEmailCurrent, // 🌟 新增
massEmailTotal, // 🌟 新增
massEmailLog, feedbackReports = [], onUpdateFeedbackReport, onDeleteFeedbackReport, }) => {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: "max-w-5xl mx-auto px-4 py-8 relative animate-fade-in-up" },
            ...
            React.createElement("div", { className: "flex gap-2 overflow-x-auto pb-4 mb-6" }, [
                ...
                t.id === "feedback" && openFeedbackCount > 0 && (React.createElement("span", { className: "bg-[#22C55E] text-[#0F111A] px-2 py-0.5 rounded-full text-xs ml-1" }, openFeedbackCount)))))),
            React.createElement("div", { className: "bg-[#181B25]/40 border border-[#2A2F3D] rounded-2xl p-6 shadow-sm" },
                ...
                activeTab === "feedback" && (React.createElement("div", { className: "space-y-4" },
                    React.createElement("div", null,
                        React.createElement("h3", { className: "text-xl font-bold text-white flex items-center gap-2" },
                            React.createElement("i", { className: "fa-solid fa-message text-[#22C55E]" }),
                            "\u529F\u80FD\u56DE\u994B\u53CA\u554F\u984C\u56DE\u5831"),
                        React.createElement("p", { className: "text-sm text-[#94A3B8] mt-2 leading-relaxed" }, "\u9996\u9801\u9001\u51FA\u7684\u56DE\u994B\u53EA\u6703\u5728\u9019\u500B\u5F8C\u53F0\u9801\u9762\u986F\u793A\uFF0C\u524D\u53F0\u4E0D\u6703\u516C\u958B\u5217\u51FA\u3002")),
                    sortedFeedbackReports.length === 0 ? (React.createElement("div", { className: "bg-[#11131C] border border-dashed border-[#2A2F3D] rounded-2xl px-5 py-10 text-center text-[#94A3B8]" }, "\u76EE\u524D\u9084\u6C92\u6709\u4F7F\u7528\u8005\u56DE\u994B\u6216\u554F\u984C\u56DE\u5831\u3002")) : (sortedFeedbackReports.map((report) => {
                        const profile = vtubers.find((v) => v.id === report.userId);
                        return (React.createElement("div", { key: report.id, className: "bg-[#11131C] border border-[#2A2F3D] rounded-2xl p-5 space-y-4" },
...
function App() {
    ...
    const handleSaveProfile = async (e, customForm = profileForm) => {
        ...
        let existingProfile = realVtubers.find((v) => v.id === targetUid);
        // ✅ 儲存前先讀取 Firestore 最新名片狀態，不只依賴 localStorage / 公開 JSON 快取。
        // 避免「待審核名片」因快取尚未更新而被誤判成新名片，造成使用者以為必須審核通過才能儲存。
        try {
            const latestProfileSnap = await getDoc(doc(db, getPath("vtubers"), targetUid));
            if (latestProfileSnap.exists()) {
                existingProfile = { id: latestProfileSnap.id, ...latestProfileSnap.data() };
            }
        }
        catch (readErr) {
            console.warn("讀取最新名片狀態失敗，將使用本機快取判斷：", readErr);
        }
        const normalizedScheduleFields = normalizeScheduleFields(customForm);
        // 🌟 保留審核防濫用：退回 >= 3 次且在 24 小時內，禁止重新提交審核。
        ...
        if (normalizedScheduleFields.isScheduleAnytime) {
            hasValidSchedule = true;
        }
        else if (normalizedScheduleFields.isScheduleCustom) {
            hasValidSchedule = normalizedScheduleFields.customScheduleText && normalizedScheduleFields.customScheduleText.trim() !== "";
        }
        else {
            // 選擇「指定時段」或「除了以下時段」時，陣列裡面必須至少有一個時段
            hasValidSchedule = normalizedScheduleFields.scheduleSlots && normalizedScheduleFields.scheduleSlots.length > 0;
        }
        if (!hasValidSchedule) {
        ...
        try {
            ...
            // --- G. 同步本地與快取 ---
            let savedPublicData = { ...publicData, id: targetUid };
            try {
                const savedSnap = await getDoc(publicRef);
                if (savedSnap.exists()) {
                    savedPublicData = { id: savedSnap.id, ...savedSnap.data() };
                }
            }
            catch (readSavedErr) {
                console.warn("讀取剛儲存的名片資料失敗，將使用本次送出的資料更新畫面：", readSavedErr);
            }
            setRealVtubers((prev) => {
                const newList = prev.map((v) => v.id === targetUid ? { ...v, ...savedPublicData } : v);
                if (!prev.find((v) => v.id === targetUid))
                    newList.push(savedPublicData);
                syncVtuberCache(newList);
                return newList;
            });
            if (targetUid === user.uid) {
                setProfileForm((prev) => ({
                    ...
                    ...savedPublicData,
                    ...normalizedScheduleFields,
                    tags: Array.isArray(savedPublicData.tags) ? savedPublicData.tags.join(", ") : savedPublicData.tags || prev.tags || "",
                    scheduleSlots: Array.isArray(savedPublicData.scheduleSlots) ? savedPublicData.scheduleSlots.map((slot) => normalizeScheduleSlot(slot)) : [],
                }));
            }
            setSelectedVTuber((prev) => prev && prev.id === targetUid ? { ...prev, ...savedPublicData } : prev);
            // ✅ ProfileEditorForm 的一般使用者表單已由 setProfileForm 同步；
            // 管理員名片編輯彈窗的 setEditingVtuber 屬於 AdminPanel 區域作用域，
            // 不可在 App 層 handleSaveProfile 直接呼叫，否則會在資料已寫入後丟出 ReferenceError，
            // 造成畫面誤顯「儲存失敗: setEditingVtuber is not defined」。
            setDoc(doc(db, getPath("settings"), "stats"), { lastGlobalUpdate: Date.now() }, { merge: true }).catch((statsErr) => {
                console.warn("已儲存名片，但全站更新通知寫入失敗：", statsErr);
            });
        ...
        React.createElement("div", { className: "vnexus-app-shell flex flex-col min-h-screen relative", "data-vnexus-current-view": currentView },
            ...
            React.createElement("main", { className: `vnexus-route-main vnexus-view-${currentView} flex-1 pb-10`, "data-vnexus-view": currentView },
                            ...
                            React.createElement("div", { className: "vnexus-vtuber-grid-actions flex flex-wrap items-center gap-3" },
                                React.createElement("h2", { className: "text-2xl font-bold text-white flex items-center gap-2" },
                                    ...
                                    user &&
                                        ...
                                        "\u96B1\u85CF\u4E2D"))),
                                React.createElement("button", { onClick: () => setIsTipsModalOpen(true), className: "vnexus-grid-btn-tips bg-[#F59E0B]/10 text-[#F59E0B] px-3 py-1.5 rounded-lg text-sm font-bold" },
                ...
                currentView === "bulletin" && (React.createElement("div", { className: "max-w-5xl mx-auto px-4 py-8 animate-fade-in-up" },
                    React.createElement("div", { className: "flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8" },
                        React.createElement("div", null,
                            React.createElement("h2", { className: "vnexus-bulletin-title text-2xl font-bold text-white flex items-center gap-2" },
                    ...
                    const renderReactionNames = (uids = []) => {
                        ...
                        return (React.createElement(React.Fragment, null,
                            visibleIds.map((uid, index) => {
                                ...
                                return (React.createElement(React.Fragment, { key: `reaction-name-${uid}` },
                                    index > 0 && React.createElement("span", { className: "text-[#64748B]" }, "\u3001"),
                                    React.createElement("button", { type: "button", onClick: (e) => {
                                            e.stopPropagation();
                                            if (profile)
                                                setSelectedVTuber(profile);
                                            navigate(`profile/${uid}`);
                                        }, className: "font-bold text-[#E2E8F0] hover:text-[#38BDF8] hover:underline underline-offset-2 transition-colors", title: "\u67E5\u770B\u540D\u7247" }, limitDisplayText(displayName, 10))));
                            }),
                            uniqueIds.length > 8 && React.createElement("span", { className: "text-[#94A3B8]" },
                                "\u3001\u7B49 ",
                                uniqueIds.length,
                                " \u4EBA")));
                    };
                                ...
                                return (React.createElement("div", { key: `story-ring-${v.id}`, className: "vnexus-story-item flex-shrink-0 w-20 text-center group", style: STORY_ITEM_STYLE, title: titleText },
                                    React.createElement("button", { type: "button", onClick: () => { if (hasActiveStory)
                                        ...
                                        React.createElement("div", { className: `w-14 h-14 mx-auto rounded-full p-[2px] ${ringClass}` },
                                            React.createElement("div", { className: "w-full h-full rounded-full bg-[#11131C] p-[2px]" },
                                                React.createElement("img", { src: sanitizeUrl(v.avatar), className: "w-full h-full rounded-full object-cover", onError: (e) => { applyVnexusImageFallback(e.currentTarget); }, alt: v.name || "創作者" })))),
                                    React.createElement("button", { type: "button", onClick: () => { if (hasActiveStory)
                        ...
                        isVerifiedUser && myProfile && (React.createElement("div", { className: "mb-4 bg-[#181B25]/55 border-y sm:border border-[#2A2F3D] sm:rounded-2xl px-4 sm:px-5 py-4" },
                            React.createElement("form", { onSubmit: async (e) => {
                                ...
                                React.createElement("button", { type: "button", onClick: () => { setSelectedVTuber(myProfile); navigate(`profile/${myProfile.id || user?.uid}`); }, className: "flex-shrink-0 mt-0.5", title: "\u67E5\u770B\u6211\u7684\u540D\u7247" },
                                    React.createElement("img", { src: sanitizeUrl(myProfile.avatar), className: "w-10 h-10 sm:w-11 sm:h-11 rounded-full object-cover border border-[#2A2F3D]", onError: (e) => { applyVnexusImageFallback(e.currentTarget); } })),
                                React.createElement("div", { className: "min-w-0 flex-1" },
                            ...
                            return (React.createElement("article", { key: v.id, className: "px-4 sm:px-5 py-4 hover:bg-[#1D2130]/55 transition-colors" },
                                React.createElement("div", { className: "flex gap-3 sm:gap-4" },
                                    React.createElement("button", { type: "button", onClick: () => { setSelectedVTuber(v); navigate(`profile/${v.id}`); }, className: "flex-shrink-0 self-start", title: "\u67E5\u770B\u540D\u7247" },
                                        React.createElement("img", { src: sanitizeUrl(v.avatar), className: `w-11 h-11 sm:w-12 sm:h-12 rounded-full object-cover border ${isLiveMsg ? 'border-[#EF4444] ring-2 ring-red-500/25' : 'border-[#2A2F3D] hover:border-[#F59E0B]/70'} transition-colors`, onError: (e) => { applyVnexusImageFallback(e.currentTarget); } })),
                                    React.createElement("div", { className: "min-w-0 flex-1" },
                                        ...
                                        React.createElement("div", { className: "mt-3 flex items-center gap-2", onClick: (e) => e.stopPropagation() },
                                            ...
                                            user && v.id === user.uid && (React.createElement("button", { onClick: (e) => { e.stopPropagation(); if (confirm("確定要清除這則動態嗎？"))
                                                ...
                                                " \u6E05\u9664"))),
                                        (fireCount > 0 || plusCount > 0) && (React.createElement("div", { className: "mt-2 space-y-1 text-[11px] leading-relaxed text-[#94A3B8]" },
                                            fireCount > 0 && (React.createElement("p", null,
                                                React.createElement("span", { className: "text-[#FCD34D] font-bold" }, "\uD83D\uDD25 \u5E6B\u63A8\uFF1A"),
                                                renderReactionNames(v.statusReactions?.fire))),
                                            plusCount > 0 && (React.createElement("p", null,
                                                React.createElement("span", { className: "text-[#C4B5FD] font-bold" }, "\uD83D\uDC4B +1\uFF1A"),
                                                renderReactionNames(v.statusReactions?.plus_one))))),
